...
<?php if (!empty($timelineItems)): ?>
	<div class="timelinebody">
        ...
        <?php
          ...
          $itemDescription[$index] = $item->options->below_text;          
        ?>
				<div class="moa-roadmap-wrapper" id='<?php echo "moa-roadmap-wrapper-".$index ?>' onclick="showmessage('<?php echo $index?>', '<?php echo $numberOfColumns?>')">
    ...
    <div class="moa-roadmap-details">      
      <?php foreach ($itemDescription as $index => $item): ?>
        <div id='<?php echo "moa-roadmap-details-content-" . $index ?>' class="moa-roadmap-details-content-hidden">
          <?php echo $item ?>
        </div>
      <?php endforeach; ?>
    </div>
